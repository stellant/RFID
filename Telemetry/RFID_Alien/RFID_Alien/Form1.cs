            ...
            try
            {
                if (data != null && data.Length > 0 && data.IndexOf("No Tags") == -1)
                {
                    ...
                    foreach (TagInfo info in tags)
                    {
                        ...
                        WriteData(info.TagData, DateTime.Now.ToString("yyyyMMddHHmmssfff") + "Tz" + convertTimeZone(TimeZone.CurrentTimeZone.GetUtcOffset(DateTime.Now).ToString()));
                        WriteText(info.TagData + "\n");
                    }
                }
            }
            catch (Exception ex)
